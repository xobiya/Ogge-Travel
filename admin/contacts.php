                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group <?= !$m['is_read'] ? 'bg-blue-50/10' : '' ?>">
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <?php if (!$m['is_read']): ?>
                                    <a href="includes/admin-actions.php?action=mark_contact_read&id=<?= $m['id'] ?>&csrf_token=<?= urlencode($admin_csrf_token) ?>" class="px-4 py-2 text-[0.65rem] font-black uppercase tracking-widest bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white transition-all">Mark Read</a>
                                <?php endif; ?>
